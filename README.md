...
D1 подключена как binding `DB`. Миграции создают аккаунты и сессии Better Auth, профили приложения, комнаты и членство с ролями `owner`, `admin` и `member`. Личный кабинет позволяет создавать комнаты и присоединяться по коду. Участники таймера, история и циклы жеребьёвки пока продолжают храниться в `localStorage` браузера — их перенос в комнаты выполняется следующим этапом.
...
1. Серверные участники, задания, таймер и история выступлений внутри комнаты.
