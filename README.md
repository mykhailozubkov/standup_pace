...
D1 подключена как binding `DB`. Миграции создают аккаунты и сессии Better Auth, профили приложения, постоянные комнаты, членство с ролями `owner`, `admin` и `member`, а также отдельные стендап-сессии внутри комнат. Личный кабинет позволяет создавать комнаты и присоединяться по коду. Участники таймера, выступления и циклы жеребьёвки пока продолжают храниться в `localStorage` браузера — их перенос во встречи выполняется следующим этапом.
...
1. Серверные выступления, задания и таймер внутри активной стендап-сессии.
2. Durable Objects и WebSocket для синхронизации комнаты в реальном времени.
